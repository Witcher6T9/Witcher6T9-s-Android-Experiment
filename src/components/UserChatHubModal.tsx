...
export type ChatHubTab = 'team-chat' | 'ai-advisor';
...
export const UserChatHubModal: React.FC<UserChatHubModalProps> = ({
  ...
  const currentChannel = DEFAULT_CHAT_CHANNELS.find(c => c.id === activeChannelId) || {
    id: activeChannelId,
    name: activeChannelId.replace('dm_', '@'),
    description: 'Direct confidential messaging',
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Primary Tabs: Floor Team Chat vs AI Advisor */}
        <div className="flex items-center border-b border-[#e7e1d5] px-4 pt-1 bg-[#f5f3ec]/60 shrink-0">
          <button
        ...
        {activeTab === 'team-chat' && (
          <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0 relative">
                  ...
                  DEFAULT_CHAT_CHANNELS.filter(c => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs whitespace-nowrap shrink-0 transition-all cursor-pointer active:scale-95 ${
                          isActive
                            ? 'bg-[#176f78] text-white font-bold shadow-xs'
                            : isIe
              ...
              <div className="hidden md:flex flex-col flex-1 overflow-y-auto p-3 space-y-4 no-scrollbar">
                ...
                {(categoryFilter === 'all' || categoryFilter === 'dm') && (
                  <div className="space-y-1 pt-2 border-t border-[#e7e1d5]">
                    <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-wider text-[#527078] px-1">
                      <span>Team Directory & Mentions</span>
                      <span className="text-[10px] text-teal-700 font-semibold">Tap @ to tag</span>
                    </div>
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-[#fbfaf6]">
              ...
              <div className="px-4 py-2.5 bg-white border-b border-[#e7e1d5] flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer active:scale-95 ${
                      isPinnedDrawerOpen
                        ? 'bg-amber-500 text-white shadow-2xs'
                        : channelPinnedMessages.length > 0
                        ? 'bg-amber-100/90 text-amber-900 border border-amber-300 hover:bg-amber-200'
                        : 'text-slate-400 hover:bg-[#f1eee6] hover:text-slate-600'
                    }`}
                    title={
                      channelPinnedMessages.length > 0
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer active:scale-95 ${
                      filterUrgentOnly
                        ...
                        : 'text-slate-500 hover:bg-[#f1eee6]'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className="px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold bg-[#dceceb] text-[#176f78] hover:bg-[#cde4e3] flex items-center gap-1 transition-colors cursor-pointer active:scale-95"
                    title="Generate executive AI shift briefing from recent floor messages"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-[#176f78]" />
                    <span className="hidden sm:inline">
                      {isGeneratingSummary ? 'Summarizing...' : 'AI Shift Summary'}
                    </span>
                  </button>
                </div>
              </div>
            ...
            {selectedMemberForCard && (
              <ChatMemberCardModal
                ...
                onMentionMember={name => handleInsertMention(name)}
